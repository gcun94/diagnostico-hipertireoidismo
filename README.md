...
1. **Reconhecimento inicial:** dataset com 30 colunas, todas carregadas como categoricas. Dados faltantes representados como `?`. TBG e TBG measured 100% nulos e removidas.
2. **Distribuicao univariada:** identificacao de outlier em `age` (valor 455), assimetria positiva em TSH, T3 e T4U.
...
Pipeline de 11 etapas com divisao clara entre operacoes deterministicas (antes do split) e estatisticas (depois do split):
...
| 5 | Remocao de 14 colunas |
| 6 | Transformacao log1p em TSH, T3, T4U |
| 7 | Split estratificado (75/25, random_state=42) |
| 8 | OneHotEncoder no referral_source |
| 9 | Tratamento de outliers (age > 115) |
| 10 | RobustScaler nas variaveis continuas |
| 11 | KNNImputer (k=5, weights=distance) |

**Features engineered:**
* `risk_factors`: contagem de 7 variaveis de burden clinico (sick, thyroid surgery, pregnant, tumor, goitre, on antithyroid medication, query hyperthyroid)
* `psych_lithium_group`: 3 categorias com logica OR para capturar pacientes com lithium sem psych documentado

**Shape final:** X_train (2829, 22) e X_test (943, 22), sem nulos.
